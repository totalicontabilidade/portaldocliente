...
(function (global) {
  ...
  function telaHistorico() {
    var e = global.Portal.empresa; Shell.titulo("Linha do tempo");
    Shell.render(UI.esqueleto(6));
    eventos().then(function (ev) {
      var porMes = U.agrupar(ev, function (x) { return U.anoMes(x.em); });
      Shell.render('<div class="pagina"><div class="cabecalho"><div><div class="cabecalho__kicker">Sua empresa na Totali</div><h1>Linha do tempo</h1><p>Tudo o que aconteceu desde o primeiro dia. É seu: exporte quando quiser (LGPD, art. 18).</p></div><div class="cabecalho__acoes"><button type="button" class="btn btn--contorno" data-acao="csv">' + ic("download") + 'Exportar CSV</button><button type="button" class="btn btn--contorno" data-acao="json">' + ic("download") + "Exportar dados (JSON)</button></div></div>" +
  ...
  function telaEntregas() {
    ...
    Dados.colListar("empresas/" + e.id + "/entregas").then(function (lista) {
      Shell.render('<div class="pagina"><div class="cabecalho"><div><div class="cabecalho__kicker">Todo mês</div><h1>Guias e relatórios</h1><p>O que a Totali entrega para a sua empresa: guias de impostos, folha, relatórios. Cada entrega registra quando você viu.</p></div></div>' + (lista.length ? '<div class="pilha" style="gap:6px">' + lista.map(function (d) { return '<div class="doc"><span class="doc__icone">' + ic("receipt") + '</span><div style="flex:1"><div class="doc__nome">' + U.esc(d.titulo) + '</div><div class="doc__meta">' + U.esc(d.competencia || "") + (d.vencimento ? " · vence " + U.data(d.vencimento) : "") + (d.vistoEm ? ' · <span class="txt-ok">visto ' + U.relativo(d.vistoEm) + "</span>" : "") + "</div></div>" + (d.url ? '<a class="btn btn--xs btn--primario" href="' + U.esc(U.urlSegura(d.url)) + '" target="_blank" rel="noopener" data-acao="visto" data-id="' + d.id + '">' + ic("download", "ic--sm") + "Abrir</a>" : "") + "</div>"; }).join("") + "</div>" : '<div class="card"><div class="card__corpo pilha"><b>Em breve, aqui.</b><p class="f-13 txt-2">A Totali está preparando o sistema de entregas da equipe. Quando ele entrar no ar, suas guias e relatórios do mês passam a chegar por esta tela, com aviso no chat. Até lá, você recebe pelo canal combinado com seu gerente.</p></div></div>') + "</div>");
