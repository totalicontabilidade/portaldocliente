...
(function (global) {
  ...
  function telaIndicacoesPainel() {
    ...
    Dados.colListar("indicacoes").then(function (lista) {
      ...
      Shell.view().onchange = function (ev) { var s = ev.target.closest("[data-acao=st]"); if (!s) return; Dados.docSalvar("indicacoes/" + s.dataset.id, { status: s.value }, true).then(function () { UI.toast("Status salvo.", "ok"); }); };
    });
  }
