...
(function (global) {
    ...
    P.prepararAuto().then(function () {
      ...
      var html = '<div class="pagina"><div class="cabecalho"><div><div class="cabecalho__kicker">Sua entrada na Totali</div><h1>Lista de documentos</h1><p>Tudo o que precisamos para assumir sua contabilidade. Cada item tem um botão para enviar e um botão de ajuda explicando o que é e onde conseguir. A maior parte vem da sua contabilidade anterior; você envia só o que é seu.</p></div>' +
        ...
        (global.Financeiro ? global.Financeiro.cartaoCadastro(e, false) : "") + '<div class="pilha">' + GRUPOS.map(function (g) {
          ...
          return '<details class="card" id="g-' + g.id + '"' + (grupoAberto === g.id || (!grupoAberto && !na && f < t) ? " open" : "") + '><summary class="card__corpo linha" style="cursor:pointer"><span class="selo-sistema" style="background:var(--primary-soft);color:var(--primary);width:36px;height:36px;border-radius:10px">' + ic(g.icone) + '</span><div class="lista__texto"><b>' + U.esc(g.titulo) + '</b><span class="lista__sub">' + (na ? "não se aplica" : f + "/" + t + (t === 1 ? " item" : " itens")) + "</span></div>" + (na ? UI.badge("não se aplica") : f === t && t ? UI.badge("completo", "ok", "check") : UI.badge(f + "/" + t, f ? "info" : "")) + '</summary><div class="card__corpo pilha" style="padding-top:0"><p class="f-13 txt-2">' + U.esc(g.desc) + "</p>" +
  ...
  function abaEntrada(e, corpo) {
    ...
    Dados.documentos(e.id).then(function (docs) {
      ...
      UI.delegar(corpo, {
        ...
        cobrar: function () { var faltam = prog.obrigFaltam.map(function (f) { return f.it.nome; }); var texto = "Oi! Para concluirmos a entrada da " + e.fantasia + " ainda faltam: " + (faltam.slice(0, 5).join(", ") || "alguns itens") + (faltam.length > 5 ? " e mais " + (faltam.length - 5) : "") + ". É só abrir a Lista de documentos no portal e enviar. Qualquer dúvida, responde por aqui"; if (global.Painel && global.Painel.mensagemPronta) return global.Painel.mensagemPronta(e, { titulo: "Cobrar a Lista de documentos · " + e.fantasia, texto: texto, assunto: "Documentos para a sua entrada na Totali", rota: "#/entrada" }); Dados.enviarMensagem(e.id, { autor: { uid: sessao.uid, nome: sessao.nome, lado: "equipe" }, texto: texto }).then(function () { UI.toast("Cobrança enviada pelo chat.", "ok"); }); },
  ...
  global.InicioExtras.push(function () {
    ...
    var progressoCard = '<a class="card card--clicavel" href="#/entrada" style="text-decoration:none;color:inherit"><div class="card__cab"><h2>Lista de documentos</h2><span class="btn btn--xs btn--contorno">Abrir</span></div><div class="card__corpo" style="padding-top:10px"><div class="linha linha--entre f-13 txt-2"><span>' + p.feitos + " de " + U.plural(p.total, "1 item", p.total + " itens") + "</span><span>" + (p.obrigFaltam.length ? U.plural(p.obrigFaltam.length, "falta 1 obrigatório", "faltam " + p.obrigFaltam.length + " obrigatórios") : "obrigatórios entregues") + "</span></div>" + UI.barra(p.pct, p.pct >= 70 ? "barra--gold" : "") + '<div class="f-12 txt-2 mt-4">O que a Totali precisa para assumir a sua contabilidade.</div></div></a>';
    return { colunaTopo: progressoCard, hoje: '<a class="card card--clicavel card--hoje entra" href="#/entrada' + (prox ? "?g=" + (prox.sid || prox.g.id) : "") + '" style="text-decoration:none;color:inherit;--tom:var(--gold);border-left:4px solid var(--gold)"><div class="card__corpo" style="display:flex;gap:14px;align-items:center"><span class="selo-sistema" style="background:var(--gold-soft);color:var(--gold-text)">' + ic("clipboard") + '</span><div style="flex:1;min-width:0"><div class="f-12 f-800 txt-2" style="letter-spacing:.08em;text-transform:uppercase">Hoje</div><div class="f-15 f-800">' + (p.feitos ? U.plural(p.faltam, "Lista de documentos: falta 1 item", "Lista de documentos: faltam " + p.faltam + " itens") : "Vamos começar pela lista de documentos") + '</div><div class="f-13 txt-2">' + (prox ? "Próximo: " + U.esc(prox.it.nome) : "Cadastre os sócios e envie o que só você tem.") + '</div></div><span class="btn btn--sm btn--primario so-desktop">Continuar · ' + p.pct + "%</span>" + ic("chevron-right", "so-mobile") + "</div></a>" };
  });
