...
(function (global) {
  ...
  function estado(e) { return Object.assign({ temBanco: null, bancos: [], bancoOutro: "", temMaquineta: null, maquinetas: [], maquinetaOutra: "", forma: "", acessos: {}, semCredencial: {}, observacoes: "", status: "", protocolo: "" }, (e && e.financeiro) || {}); }
  function completo(f) {
  ...
  function passoAtual(f) { if (f.temBanco === null || (f.temBanco && !f.bancos.length && !f.bancoOutro)) return 1; if (f.temMaquineta === null || (f.temMaquineta && !f.maquinetas.length && !f.maquinetaOutra)) return 2; if (f.temMaquineta && !f.forma) return 3; return completo(f) ? 4 : 3; }
  ...
  function telaFinanceiro(r) {
    ...
    if (passo === 4) corpo = '<h2>Revisão</h2><div class="pilha mt-8" style="gap:6px"><div class="doc"><span class="doc__icone">' + ic("building") + '</span><div><div class="doc__nome">Bancos</div><div class="doc__meta">' + (f.temBanco ? U.esc(f.bancos.concat(f.bancoOutro ? [f.bancoOutro] : []).join(", ")) : "sem conta bancária") + '</div></div></div><div class="doc"><span class="doc__icone">' + ic("credit-card") + '</span><div><div class="doc__nome">Maquininhas</div><div class="doc__meta">' + (f.temMaquineta ? U.esc(f.maquinetas.concat(f.maquinetaOutra ? [f.maquinetaOutra] : []).join(", ")) + " · " + (f.forma === "acesso" ? "a Totali baixa os relatórios" : "você envia os relatórios todo mês") : "não usa") + '</div></div></div></div><div class="campo mt-12"><label class="campo__rotulo">Observações (opcional)</label><textarea class="textarea" id="obs">' + U.esc(f.observacoes) + "</textarea></div>" + (f.status ? '<div class="aviso aviso--ok mt-12">' + ic("check-circle") + "<div><b>Enviado " + U.relativo(f.enviadoEm) + ".</b> Protocolo " + U.esc(f.protocolo) + ". Você pode alterar e reenviar quando quiser." + (f.termo ? ' <a href="#/documentos">Ver o termo em Documentos.</a>' : "") + "</div></div>" : "");
    Shell.render('<div class="pagina" style="max-width:760px"><div class="cabecalho"><div><div class="cabecalho__kicker">Checklist financeiro · passo ' + passo + ' de 4</div><h1>Bancos e maquininhas</h1><p>Onde a empresa movimenta dinheiro e como vamos receber os relatórios de venda. Leva uns 2 minutos.</p></div></div>' + UI.barra(passo * 25, "barra--gold") + '<div class="card"><div class="card__corpo pilha">' + corpo + '</div></div><div class="linha linha--entre">' + (passo > 1 ? '<button type="button" class="btn btn--contorno" data-acao="ir" data-p="' + (passo - 1) + '">' + ic("arrow-left") + "Voltar</button>" : "<span></span>") + (passo < 4 ? '<button type="button" class="btn btn--primario" data-acao="ir" data-p="' + (passo + 1) + '">Continuar' + ic("arrow-right") + "</button>" : '<button type="button" class="btn btn--gold" data-acao="enviar">' + ic("check") + (f.status ? "Reenviar" : "Enviar para a Totali") + "</button>") + "</div></div>");
    var v = Shell.view();
    function guardar(depois) { var bo = UI.$("#bancoOutro", v), mo = UI.$("#maquinetaOutra", v), ob = UI.$("#obs", v); if (bo) f.bancoOutro = U.txt(bo.value, 80); if (mo) f.maquinetaOutra = U.txt(mo.value, 80); if (ob) f.observacoes = U.txt(ob.value, 1000); return Dados.salvarEmpresa(e.id, { financeiro: f }).then(P.recarregar).then(depois); }
  ...
  function abaFinanceiro(e, corpo) {
    ...
    UI.delegar(corpo, {
      ...
      cobrar: function () { Dados.enviarMensagem(e.id, { autor: { uid: sessao.uid, nome: sessao.nome, lado: "equipe" }, texto: "Oi! Para organizar o financeiro da " + e.fantasia + ", pedimos que responda o checklist de bancos e maquininhas no portal: são duas perguntas rápidas, leva uns 2 minutos e dá para fazer pelo celular. Fica em Entrada na Totali › Bancos e maquininhas 🙂" }).then(function () { UI.toast("Cobrança enviada.", "ok"); }); }
    });
  ...
  global.Financeiro = { estado: estado, completo: completo, BANCOS: BANCOS, MAQUINETAS: MAQUINETAS, TERMO: TERMO, RELATORIOS: RELATORIOS };
})(window);
