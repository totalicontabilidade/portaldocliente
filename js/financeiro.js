...
(function (global) {
  ...
  function simNaoTexto(v) { return v === "sim" ? "sim" : v === "nao" ? "não" : U.esc(v || "—"); }
  ...
  function telaFinanceiro(r) {
    ...
    if (passo > 5) passo = 5;
    /* sem maquininha não existe o passo 3 (como recebemos os relatórios): pula na direção em que a pessoa ia */
    if (passo === 3 && f.temMaquineta === false) passo = Number(r.query.de) === 4 ? 2 : 4;
  ...
  function abaFinanceiro(e, corpo) {
    ...
    UI.delegar(corpo, {
      ...
      cobrar: function () { var texto = "Olá! Para facilitar o cadastro e a organização das informações da " + e.fantasia + ", pedimos que você preencha o Checklist Financeiro aqui no portal, em Bancos e maquininhas. São perguntas rápidas: quais bancos a empresa utiliza e quais maquininhas de cartão. Leva cerca de 2 minutos e pode ser feito pelo celular. Essas informações são importantes para a conferência e o acompanhamento dos dados para a contabilização da empresa. Link: " + U.urlPortal("index.html#/financeiro"); if (global.Painel && global.Painel.mensagemPronta) return global.Painel.mensagemPronta(e, { titulo: "Cobrar o Checklist Financeiro · " + e.fantasia, texto: texto, assunto: "Checklist Financeiro: bancos e maquininhas", rota: "#/financeiro" }); Dados.enviarMensagem(e.id, { autor: { uid: sessao.uid, nome: sessao.nome, lado: "equipe" }, texto: texto }).then(function () { UI.toast("Cobrança enviada.", "ok"); }); }
    });
  ...
  /* Cartão do cadastro: bancos e maquininhas é passo do onboarding (Raoni, 23/09/2026: "essas informações são
     importantes"). Aparece no topo da tela inicial e na Lista de documentos até o cliente enviar. */
  function cartaoCadastro(e, destaque) {
    var f = estado(e); if (f.status) return "";
    var p = passoAtual(f), comecou = f.temBanco !== null;
    return '<a class="card card--clicavel' + (destaque ? " card--hoje" : "") + '" href="#/financeiro" style="text-decoration:none;color:inherit;border-left:4px solid var(--gold)"><div class="card__corpo linha cartao-cadastro" style="gap:14px"><span class="selo-sistema" style="background:var(--gold-soft);color:var(--gold-text)">' + ic("credit-card") + '</span><div class="lista__texto" style="flex:1"><span class="f-12 f-800 txt-2" style="letter-spacing:.08em;text-transform:uppercase">Cadastro · passo importante</span><b>Bancos e maquininhas</b><span class="lista__sub">' + (comecou ? "Você parou no passo " + p + " de 5. Continue de onde parou." : "Em quais bancos a empresa tem conta, se usa maquininha e como a Totali recebe os relatórios. Leva uns 3 minutos.") + "</span></div>" + '<span class="btn btn--sm btn--primario">' + (comecou ? "Continuar" : "Responder") + "</span></div></a>";
  }
  global.InicioExtras.push(function () {
    var e = global.Portal.empresa, c = cartaoCadastro(e, false);
    return c ? { topo: c } : null;
  });
  ...
  global.Financeiro = { estado: estado, completo: completo, cartaoCadastro: cartaoCadastro, aplicarCatalogo: aplicarCatalogo, BANCOS: BANCOS, MAQUINETAS: MAQUINETAS, TERMO: TERMO, RELATORIOS: RELATORIOS };
})(window);
