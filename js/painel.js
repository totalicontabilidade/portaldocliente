...
(function (global) {
  ...
  function telaVitrine() {
    ...
    Promise.all([Dados.vitrine(), Dados.usos({ desde: Date.now() - 30 * U.DIA_MS })]).then(function (r) {
      ...
      var card = function (c, padrao) { var s = CATALOGO.por(c.sistemaId); var st = stats(c.id); return '<div class="card sistema"><div class="sistema__topo"><span class="selo-sistema" style="background:' + s.cor + '">' + ic(s.icone) + '</span><div style="flex:1;min-width:0"><div class="sistema__nome">' + U.esc(c.titulo) + '</div><div class="sistema__tag">' + U.esc(s.nome) + " · " + (c.publico === "todos" ? "todas as empresas" : "quem não tem o sistema") + (c.gatilho ? " · por gatilho no chat" : "") + "</div></div>" + (c.ativo ? UI.badge("ativa", "ok") : UI.badge("pausada")) + '</div>' + (c.imagem && c.imagem.url ? '<img class="vitrine__miniatura" src="' + U.esc(c.imagem.url) + '" alt="' + U.esc(c.titulo) + '">' : "") + '<div class="sistema__desc">' + U.esc(c.texto) + '</div><div class="f-12 txt-2">' + st.impressao + " impressões · " + st.clique + " cliques · " + st.fechou + " fechou · " + st.interesse + ' pedidos</div><div class="sistema__acoes">' + (padrao ? '<span class="badge">padrão do código</span><button type="button" class="btn btn--xs btn--contorno" data-acao="clonar" data-id="' + c.id + '">' + ic("copy", "ic--sm") + "Copiar e editar</button>" : '<button type="button" class="btn btn--xs btn--contorno" data-acao="editar" data-id="' + c.id + '">' + ic("pencil", "ic--sm") + 'Editar</button><button type="button" class="btn btn--xs btn--fantasma" data-acao="remover" data-id="' + c.id + '">' + ic("trash", "ic--sm") + "</button>") + "</div></div>"; };
