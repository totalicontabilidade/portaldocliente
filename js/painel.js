...
(function (global) {
  ...
  function telaInicio() {
    ...
    Promise.all([Dados.todasConversas(), Dados.todosDocumentos(), Dados.usos({ desde: semana }), Dados.listarChecklists(U.anoMes(Date.now()))]).then(function (r) {
      ...
      var porEmpCheck = U.porChave(checks, "empresaId"), anoMesAtual = U.anoMes(Date.now());
      var envios = global.Envio ? empresas.filter(function (e) { return e.ativa !== false; }).map(function (e) { return global.Envio.mes(e, porEmpCheck[e.id], anoMesAtual); }).filter(function (c) { return c.itens.length; }) : [];
      var checkOk = envios.filter(function (c) { return c.concluidoEm; }).length;
      /* cadastro que depende do cliente: Lista de documentos e Checklist Financeiro */
      var ativas = empresas.filter(function (e) { return e.ativa !== false; });
      var entradaFalta = global.Onboarding ? ativas.filter(function (e) { var pr = global.Onboarding.progresso(e); return !pr.completo && !e.migracaoConcluidaEm && pr.obrigFaltam.length; }).length : 0;
      var financeiroFalta = global.Financeiro ? ativas.filter(function (e) { return !global.Financeiro.completo(global.Financeiro.estado(e)); }).length : 0;
      ...
      Shell.render('<div class="pagina"><div class="cabecalho"><div><div class="cabecalho__kicker">' + U.esc(U.diaSemana(Date.now())) + "</div><h1>" + U.saudacao() + ", " + U.esc(U.primeiroNome(sessao.nome)) + "</h1><p>O que precisa de você hoje, em ordem de urgência.</p></div><div class=\"cabecalho__acoes\"><a class=\"btn btn--primario\" href=\"#/clientes/novo\">" + ic("plus") + "Novo cliente</a></div></div>" +
        ...
        '</div><div class="pilha">' +
          ...
          (entradaFalta || financeiroFalta ? '<div class="card"><div class="card__cab"><h2>Cadastro dos clientes</h2></div><div class="lista" style="padding-top:6px">' +
            (entradaFalta ? '<a class="lista__item" href="#/entrada">' + ic("clipboard") + '<div class="lista__texto"><span class="lista__titulo">Lista de documentos</span><span class="lista__sub">' + U.plural(entradaFalta, "1 cliente com obrigatório faltando", entradaFalta + " clientes com obrigatório faltando") + "</span></div>" + ic("chevron-right", "ic--sm") + "</a>" : "") +
            (financeiroFalta ? '<a class="lista__item" href="#/financeiro">' + ic("credit-card") + '<div class="lista__texto"><span class="lista__titulo">Bancos e maquininhas</span><span class="lista__sub">' + U.plural(financeiroFalta, "1 cliente não respondeu", financeiroFalta + " clientes não responderam") + "</span></div>" + ic("chevron-right", "ic--sm") + "</a>" : "") +
            "</div></div>" : "") +
          '<div class="card"><div class="card__cab"><h2>Envio de ' + nomeMes(U.anoMes(Date.now())).split(" de ")[0] + '</h2><a class="btn btn--xs btn--contorno" href="#/checklist">Detalhar</a></div><div class="card__corpo" style="padding-top:10px"><div class="f-800" style="font-size:26px">' + checkOk + ' <small class="f-13 txt-2">de ' + envios.length + " concluíram</small></div>" + UI.barra(U.pct(checkOk, envios.length || 1), "barra--ok") + "</div></div>" +
          '<div class="card card--navy"><div class="puzzle-layer" aria-hidden="true"></div><div class="veu"></div><div class="card__corpo pilha"><div class="f-12" style="color:var(--sidebar-muted);font-weight:800;letter-spacing:.1em;text-transform:uppercase">Atalhos</div><a class="btn btn--sm btn--gold" href="#/clientes/novo">' + ic("plus") + 'Cadastrar cliente</a><a class="btn btn--sm btn--contorno" style="background:rgba(255,255,255,.08);color:#fff;border-color:rgba(255,255,255,.2)" href="#/vitrine">' + ic("megaphone") + 'Nova campanha</a><a class="btn btn--sm btn--contorno" style="background:rgba(255,255,255,.08);color:#fff;border-color:rgba(255,255,255,.2)" href="#/uso">' + ic("bar-chart") + "Uso e cobrança</a></div></div>" +
        "</div></div></div>");
    });
  ...
  function mensagemPronta(e, o) {
    ...
    UI.modal({ titulo: o.titulo || "Mensagem pronta", larga: true, corpo: '<p class="f-13 txt-2">Revise o texto e escolha por onde mandar. Dá para usar mais de um.</p><textarea class="textarea" id="mpTexto" style="min-height:170px">' + U.esc(o.texto) + '</textarea><div class="f-12 txt-2 mt-4">' + ic("whatsapp", "ic--sm") + " " + (num ? "WhatsApp do cliente: " + whatsappTexto(num) : "Sem WhatsApp do cliente: o WhatsApp abre para você escolher o contato. Cadastre na ficha, em Editar.") + "</div>",
      acoes: [
        { rotulo: "Copiar texto", icone: "copy", classe: "btn--fantasma btn--sm", manter: true, ao: function (c) { UI.copiar(c.querySelector("#mpTexto").value, "Texto copiado."); } },
  ...
  function abaJornada(e, corpo) {
    ...
    carregarAuto(e).then(function () {
      ...
      UI.delegar(corpo, {
        "pedir-feedback": function () { Dados.enviarMensagem(e.id, { autor: { uid: sessao.uid, nome: sessao.nome, lado: "equipe" }, texto: "Oi! Fechamos seus primeiros 30 dias com a Totali. Pode me contar em uma frase como foi? É só tocar aqui: " + U.urlPortal("index.html#/feedback") }).then(function () { UI.toast("Pedido enviado pelo chat.", "ok"); }); },
        passo: function (b) {
  ...
  function telaJornadas() {
    ...
    Promise.all(empresas.map(carregarAuto)).then(function () {
      ...
      Shell.render('<div class="pagina pagina--larga"><div class="cabecalho"><div><div class="cabecalho__kicker">Onboarding</div><h1>Jornadas de 30 dias</h1><p>' + abertas.length + " em andamento · " + concluidas.length + ' concluídas. Cada coluna é um dia (D); a célula mostra os passos feitos da equipe.</p></div><a class="btn btn--sm btn--contorno" href="#/conteudo">' + ic("pencil") + "Editar a jornada</a></div>" +
        '<div class="tabela-wrap"><table class="tabela tabela--compacta"><thead><tr><th style="min-width:200px">Cliente</th><th>Hoje</th>' + JORNADA.DIAS.map(function (d) { return '<th class="centro">D' + d.dia + "</th>"; }).join("") + "<th>Cliente fez</th></tr></thead><tbody>" + abertas.map(function (x) {
  ...
  function telaChecklist(r) {
    ...
    Dados.listarChecklists(anoMes).then(function (checks) {
      ...
      Shell.render('<div class="pagina"><div class="cabecalho"><div><div class="cabecalho__kicker">Todo mês</div><h1>Envio de ' + nomeMes(anoMes) + '</h1><p>Quais empresas já mandaram tudo, quais estão paradas e o que falta em cada uma.</p></div><div class="cabecalho__acoes"><select class="select" id="selMes" style="min-height:36px;width:auto">' + meses.map(function (m) { return '<option value="' + m + '"' + (m === anoMes ? " selected" : "") + ">" + nomeMes(m) + "</option>"; }).join("") + '</select><button type="button" class="btn btn--contorno" data-acao="csv">' + ic("download") + "CSV</button></div></div>" +
        ...
        '<div class="tabela-wrap"><table class="tabela"><thead><tr><th style="min-width:140px">Empresa</th><th>Responsáveis</th><th style="min-width:140px">Progresso</th><th style="min-width:170px">Faltando</th><th>Situação</th><th style="width:1%"></th></tr></thead><tbody>' + linhas.map(function (l) { var faltam = l.c ? l.c.itens.filter(function (i) { return !i.feito; }).map(function (i) { return i.texto; }) : ["tudo"]; return '<tr><td><b>' + U.esc(l.e.fantasia) + '</b></td><td class="f-13">' + respHtml(l.e) + '</td><td><div class="f-12 txt-2">' + l.f + "/" + l.t + "</div>" + UI.barra(l.pct, l.ok ? "barra--ok" : "") + '</td><td class="f-12 txt-2">' + U.esc(faltam.slice(0, 2).join(", ")) + (faltam.length > 2 ? " +" + (faltam.length - 2) : "") + "</td><td>" + (l.nada ? UI.badge("nada este mês") : l.ok ? UI.badge("em dia", "ok", "check") : l.f === 0 ? UI.badge("não começou", "aviso") : UI.badge("em andamento", "info")) + '</td><td><div class="pilha" style="gap:4px;align-items:stretch;min-width:120px">' + (l.nada ? "" : '<button type="button" class="btn btn--xs btn--contorno" data-acao="cobrar-envio" data-id="' + l.e.id + '">' + ic("chat", "ic--sm") + "Cobrar</button>") + (l.c && !l.nada ? '<button type="button" class="btn btn--xs btn--contorno" data-acao="aceitar" data-id="' + l.e.id + '">' + ic("check", "ic--sm") + "Aceitar itens</button>" : "") + "</div></td></tr>"; }).join("") + "</tbody></table></div>" +
