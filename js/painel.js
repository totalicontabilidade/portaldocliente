...
(function (global) {
  ...
  function telaNovoCliente() {
    ...
    Shell.render('<div class="pagina" style="max-width:720px"><div class="cabecalho"><div><div class="cabecalho__kicker">Cadastro</div><h1>Novo cliente</h1><p>Cadastre a empresa e gere o link de convite. O D0 da jornada nasce marcado: proposta aceita e cadastro criado.</p></div></div>' +
      '<form class="card" id="fNovo" novalidate><div class="card__corpo pilha">' +
        ...
        '<div class="campo"><span class="campo__rotulo">Liberar de início</span><div class="linha">' + CATALOGO.visiveis().filter(function (s) { return s.status !== "breve"; }).map(function (s) { return '<label class="checar"><input type="checkbox" name="lib" value="' + s.id + '"' + (s.id === "academy" ? " checked" : "") + "> " + U.esc(s.nome) + "</label>"; }).join("") + '</div><span class="campo__ajuda">O Envio do mês já vem para todo cliente (é parte do portal). Academy sempre aberto.</span></div>' +
  ...
  function telaVitrine() {
    ...
    Promise.all([Dados.vitrine(), Dados.usos({ desde: Date.now() - 30 * U.DIA_MS })]).then(function (r) {
      ...
      var card = function (c, padrao) { var s = CATALOGO.por(c.sistemaId); var st = stats(c.id); return '<div class="card sistema"><div class="sistema__topo">' + CATALOGO.selo(s) + '<div style="flex:1;min-width:0"><div class="sistema__nome">' + U.esc(c.titulo) + '</div><div class="sistema__tag">' + U.esc(s.nome) + " · " + (c.empresas && c.empresas.length ? U.plural(c.empresas.length, "só 1 empresa escolhida", "só " + c.empresas.length + " empresas escolhidas") : c.publico === "todos" ? "todas as empresas" : "quem não tem o sistema") + (c.gatilho ? " · por gatilho no chat" : "") + "</div></div>" + (c.ativo ? UI.badge("ativa", "ok") : UI.badge("pausada")) + '</div>' + (c.imagem && c.imagem.url ? '<img class="vitrine__miniatura" src="' + U.esc(c.imagem.url) + '" alt="' + U.esc(c.titulo) + '"' + (c.foco ? ' style="object-position:' + Number(c.foco.x) + "% " + Number(c.foco.y) + '%"' : "") + ">" : "") + '<div class="sistema__desc">' + U.esc(c.texto) + '</div><div class="f-12 txt-2">' + U.plural(st.impressao, "1 impressão", st.impressao + " impressões") + " · " + U.plural(st.clique, "1 clique", st.clique + " cliques") + " · " + st.fechou + " fechou · " + U.plural(st.interesse, "1 pedido", st.interesse + " pedidos") + '</div><div class="sistema__acoes">' + (padrao ? '<span class="badge">padrão do código</span><button type="button" class="btn btn--xs btn--contorno" data-acao="clonar" data-id="' + c.id + '">' + ic("copy", "ic--sm") + "Copiar e editar</button>" : '<button type="button" class="btn btn--xs btn--contorno" data-acao="editar" data-id="' + c.id + '">' + ic("pencil", "ic--sm") + 'Editar</button><button type="button" class="btn btn--xs btn--fantasma" data-acao="remover" data-id="' + c.id + '" aria-label="Remover campanha" title="Remover campanha">' + ic("trash", "ic--sm") + "</button>") + "</div></div>"; };
