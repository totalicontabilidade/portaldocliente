...
(function (global) {
  ...
  global.Painel = {
    ...
    admin: admin, autoFnDe: autoFnDe, carregarAuto: carregarAuto, recarregar: carregarEmpresas, mostrarConvite: mostrarConvite, nomeMes: nomeMes,
  ...
  function frasePara(a) {
    var nomes = {}; (CATALOGO.SISTEMAS || []).forEach(function (s) { nomes[s.id] = s.nome; });
  ...
  function telaChecklist(r) {
    ...
    Dados.listarChecklists(anoMes).then(function (checks) {
      ...
      var linhas = comChecklist.map(function (e) { var c = global.Envio.mes(e, porEmp[e.id], anoMes); var f = c.itens.filter(function (i) { return i.feito; }).length, t = c.itens.length; return { e: e, c: c, f: f, t: t, pct: U.pct(f, t), ok: !!c.concluidoEm }; }).sort(function (a, b) { return a.pct - b.pct; });
      ...
      Shell.render('<div class="pagina"><div class="cabecalho"><div><div class="cabecalho__kicker">Todo mês</div><h1>Envio de ' + nomeMes(anoMes) + '</h1><p>Quais empresas já mandaram tudo, quais estão paradas e o que falta em cada uma.</p></div><div class="cabecalho__acoes"><select class="select" id="selMes" style="min-height:36px;width:auto">' + meses.map(function (m) { return '<option value="' + m + '"' + (m === anoMes ? " selected" : "") + ">" + nomeMes(m) + "</option>"; }).join("") + '</select><button type="button" class="btn btn--contorno" data-acao="csv">' + ic("download") + "CSV</button></div></div>" +
        ...
        '<div class="tabela-wrap"><table class="tabela"><thead><tr><th>Empresa</th><th>Responsáveis</th><th style="min-width:160px">Progresso</th><th>Faltando</th><th>Situação</th><th></th></tr></thead><tbody>' + linhas.map(function (l) { var faltam = l.c ? l.c.itens.filter(function (i) { return !i.feito; }).map(function (i) { return i.texto; }) : ["tudo"]; return '<tr><td><b>' + U.esc(l.e.fantasia) + '</b></td><td class="f-13">' + respHtml(l.e) + '</td><td><div class="f-12 txt-2">' + l.f + "/" + l.t + "</div>" + UI.barra(l.pct, l.ok ? "barra--ok" : "") + '</td><td class="f-12 txt-2">' + U.esc(faltam.slice(0, 2).join(", ")) + (faltam.length > 2 ? " +" + (faltam.length - 2) : "") + "</td><td>" + (l.ok ? UI.badge("em dia", "ok", "check") : l.f === 0 ? UI.badge("não começou", "aviso") : UI.badge("em andamento", "info")) + '</td><td><div class="linha" style="gap:4px;flex-wrap:nowrap"><a class="btn btn--xs btn--contorno" href="#/mensagens/' + l.e.id + '?cobrar=envio&mes=' + anoMes + '">' + ic("chat", "ic--sm") + "Cobrar</a>" + (l.c ? '<button type="button" class="btn btn--xs btn--contorno" data-acao="aceitar" data-id="' + l.e.id + '">' + ic("check", "ic--sm") + "Aceitar itens</button>" : "") + "</div></td></tr>"; }).join("") + "</tbody></table></div>" +
  ...
  function telaVitrine() {
    ...
    Promise.all([Dados.vitrine(), Dados.usos({ desde: Date.now() - 30 * U.DIA_MS })]).then(function (r) {
      ...
      var card = function (c, padrao) { var s = CATALOGO.por(c.sistemaId); var st = stats(c.id); return '<div class="card sistema"><div class="sistema__topo"><span class="selo-sistema" style="background:' + s.cor + '">' + ic(s.icone) + '</span><div style="flex:1;min-width:0"><div class="sistema__nome">' + U.esc(c.titulo) + '</div><div class="sistema__tag">' + U.esc(s.nome) + " · " + (c.empresas && c.empresas.length ? U.plural(c.empresas.length, "só 1 empresa escolhida", "só " + c.empresas.length + " empresas escolhidas") : c.publico === "todos" ? "todas as empresas" : "quem não tem o sistema") + (c.gatilho ? " · por gatilho no chat" : "") + "</div></div>" + (c.ativo ? UI.badge("ativa", "ok") : UI.badge("pausada")) + '</div>' + (c.imagem && c.imagem.url ? '<img class="vitrine__miniatura" src="' + U.esc(c.imagem.url) + '" alt="' + U.esc(c.titulo) + '">' : "") + '<div class="sistema__desc">' + U.esc(c.texto) + '</div><div class="f-12 txt-2">' + U.plural(st.impressao, "1 impressão", st.impressao + " impressões") + " · " + U.plural(st.clique, "1 clique", st.clique + " cliques") + " · " + st.fechou + " fechou · " + U.plural(st.interesse, "1 pedido", st.interesse + " pedidos") + '</div><div class="sistema__acoes">' + (padrao ? '<span class="badge">padrão do código</span><button type="button" class="btn btn--xs btn--contorno" data-acao="clonar" data-id="' + c.id + '">' + ic("copy", "ic--sm") + "Copiar e editar</button>" : '<button type="button" class="btn btn--xs btn--contorno" data-acao="editar" data-id="' + c.id + '">' + ic("pencil", "ic--sm") + 'Editar</button><button type="button" class="btn btn--xs btn--fantasma" data-acao="remover" data-id="' + c.id + '" aria-label="Remover campanha" title="Remover campanha">' + ic("trash", "ic--sm") + "</button>") + "</div></div>"; };
      ...
      function editor(c) {
        ...
        var novaImagem = null, tirarImagem = false;
        setTimeout(function () {
          var inp = document.getElementById("vImg"), box = document.getElementById("vImgBox"), tirar = document.getElementById("vImgTirar");
          if (inp) inp.addEventListener("change", function () {
            var f = inp.files[0]; if (!f) return;
            if (!/^image\/(png|jpeg|webp)$/.test(f.type)) return UI.toast("Use PNG, JPG ou WebP.", "aviso");
            reduzirImagem(f, 1600).then(function (blob) { novaImagem = blob; tirarImagem = false; box.innerHTML = '<img src="' + URL.createObjectURL(blob) + '" alt="">'; UI.toast("Imagem pronta (" + Math.round(blob.size / 1024) + " KB). Salve para publicar.", "ok"); }).catch(function () { UI.toast("Não consegui ler essa imagem.", "erro"); });
          });
          if (tirar) tirar.addEventListener("click", function () { tirarImagem = true; novaImagem = null; box.innerHTML = '<span class="f-12 txt-2">A imagem sai quando você salvar.</span>'; });
        }, 0);
        UI.modal({ titulo: c.titulo ? "Editar campanha" : "Nova campanha", corpo: '<div class="pilha"><div class="campo"><label class="campo__rotulo">Sistema</label><select class="select" id="vS">' + CATALOGO.SISTEMAS.map(function (s) { return '<option value="' + s.id + '"' + (s.id === c.sistemaId ? " selected" : "") + ">" + U.esc(s.nome) + "</option>"; }).join("") + '</select></div>' + '<div class="campo"><span class="campo__rotulo">Imagem do banner (opcional)</span><div class="imagem-banner" id="vImgBox">' + (c.imagem && c.imagem.url ? '<img src="' + U.esc(c.imagem.url) + '" alt="">' : '<span class="f-12 txt-2">Sem imagem: o banner usa o texto e as cores do sistema.</span>') + '</div><div class="linha" style="gap:6px"><label class="btn btn--sm btn--contorno" style="cursor:pointer">' + ic("upload", "ic--sm") + (c.imagem && c.imagem.url ? "Trocar imagem" : "Escolher imagem") + '<input type="file" id="vImg" accept="image/png,image/jpeg,image/webp" hidden></label>' + (c.imagem && c.imagem.url ? '<button type="button" class="btn btn--sm btn--fantasma" id="vImgTirar">' + ic("trash", "ic--sm") + "Tirar imagem</button>" : "") + '</div><span class="campo__ajuda">Arte pronta, feita por vocês: 1200 × 500 px (PNG, JPG ou WebP). Com imagem, o banner mostra só a arte, clicável; no menu lateral ela aparece recortada no centro, então deixe o principal no meio. O título abaixo vira a descrição da imagem para leitores de tela.</span></div>' +
  ...
  function telaUso(r) {
    ...
    Dados.usos({ desde: Date.now() - dias * U.DIA_MS, sistemaId: sis || undefined, empresaId: emp || undefined }).then(function (usos) {
      ...
      Shell.render('<div class="pagina pagina--larga"><div class="cabecalho"><div><div class="cabecalho__kicker">Auditoria de uso</div><h1>Uso e cobrança</h1><p>Quem está usando o portal e cada sistema, quantas vezes, por quanto tempo e em que aparelho. Exporte para informar e cobrar.</p></div><div class="cabecalho__acoes"><button type="button" class="btn btn--primario" data-acao="csv">' + ic("download") + 'Exportar CSV</button><button type="button" class="btn btn--contorno" data-acao="cobranca">' + ic("receipt") + "Relatório de cobrança</button></div></div>" +
        ...
        '<div class="tabela-wrap"><table class="tabela"><thead><tr><th>Empresa</th><th>Sistema</th><th class="num">Aberturas</th><th class="num">Minutos</th><th class="num">Dias ativos</th><th class="num">Pessoas</th><th>Último uso</th><th>Aparelho</th></tr></thead><tbody>' + (ag.map(function (x) { var s = CATALOGO.por(x.sistemaId); return '<tr><td><a href="#/clientes/' + x.empresaId + '/uso"><b>' + U.esc(x.empresa) + "</b></a></td><td>" + U.esc(s ? s.nome : x.sistemaId === "portal" ? "Portal (telas)" : x.sistemaId) + '</td><td class="num">' + x.aberturas + '</td><td class="num">' + Math.round(x.segundos / 60) + '</td><td class="num">' + x.diasAtivos + '</td><td class="num">' + x.qtdPessoas + '</td><td class="f-12">' + U.relativo(x.ultimo) + '</td><td class="f-12">' + (x.celular > x.computador ? "celular" : "computador") + "</td></tr>"; }).join("") || '<tr><td colspan="8" class="txt-2">Sem uso no período.</td></tr>') + "</tbody></table></div>" +
